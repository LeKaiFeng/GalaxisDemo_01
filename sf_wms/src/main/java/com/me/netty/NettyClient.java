...
public class NettyClient {
        ...
        do {
            ...
            try {
                ...
                future.addListener((GenericFutureListener<ChannelFuture>) f -> {
                    log.info("Netty Client 连接成功, result:{}, ip: {}, port: {}", f.isSuccess(), host, port);
                    this.channel = f.channel();
                    keep();
                });
                future.channel().closeFuture().sync();
                log.info("本地Netty Client 关闭");

            } catch (Exception e) {
                log.warn("连接远端server失败, e: {}", e.getMessage());
            } finally {
                group.shutdownGracefully();
                log.info("Netty Client资源已释放...");
    ...
    private void keep() {
        /*while (reConn) {
            log.info("channel Registered: {}, Active: {}, Open: {}, Writable: {}"
