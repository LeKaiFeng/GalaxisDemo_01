...
@Component
public class TaskFinishListen implements ApplicationListener<TaskFinishEvent> {
    private static final Logger log = LoggerFactory.getLogger(TaskFinishListen.class);
    ...
    public void onApplicationEvent(TaskFinishEvent event) {
        ...
        if (!Objects.equals(TaskEvent.EVENT_FINISH, taskEvent.getEvent())) {
            return;
        }

        log.info("处理task的完成事件, event: {}", taskEvent);
